...
const AllTasks = () => {
  ...
  const SCRIPT_URL = GLOBAL_CONFIG.APPS_SCRIPT_URL
  const SHEET_NAME = "DATA"
  const SHEET_ID = GLOBAL_CONFIG.SPREADSHEET_ID
    ...
    const fetchTasksFromGoogleSheets = async () => {
      try {
        ...
        if (data && data.table) {
          ...
          const colEId = data.table.cols[colEIndex]?.id || 'colE';
          const colLId = data.table.cols[colLIndex]?.id || 'colL';
          const colMId = data.table.cols[colMIndex]?.id || 'colM';
          ...
          const filteredTasks = data.table.rows
            .map(row => {
              const task = {
                _id: row._id,
              ...
              data.table.cols.forEach((col, i) => {
                task[col.id] = row.c[i] ? row.c[i].v : null;
              });
              return task;
            })
            .filter(task => {
          ...
          const visibleHeaders = data.table.cols.filter((header, index) =>
            index >= 1 && index <= 10
          )
  ...
  const uploadFile = async (taskId, file) => {
    ...
    try {
      ...
      reader.onload = async (e) => {
        ...
        formData.append('folderId', GLOBAL_CONFIG.DRIVE_FOLDER_ID)

        try {
          const response = await fetch(`${SCRIPT_URL}?action=uploadFile`, {
            method: 'POST',
            body: formData
          })
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const formData = new FormData()
      formData.append('sheetName', SHEET_NAME)
      formData.append('updates', JSON.stringify(tasksToUpdate))
